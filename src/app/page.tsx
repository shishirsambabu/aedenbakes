...
type CustomerDetail = {
  ...
  productionBatches: Array<{
    ...
    serviceDate: string;
    status: 'draft' | 'locked' | 'in_progress' | 'completed';
    lines: Array<{ productId: string; slotId: string; quantity: number }>;
  }>;
};
...
export default function Home() {
  ...
  async function refreshAppState(sessionToken = token, canSyncErp = session?.permissions.canSyncErp ?? false) {
    const [customersRes, catalogRes, ordersRes, operationsRes] = await Promise.all([
  ...
  async function loadCustomerDetail(customerId: string) {
    ...
    const json = (await response.json()) as CustomerDetail;
  ...
  const { customers, products, capacities, slots, orders, auditEvents, erpSyncStatus, approvals, erpContractPreview } = appState;
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
          ...
          <aside className="space-y-6">
            <Card title="Customer detail" subtitle="Open one account to inspect orders and batch impact.">
              {selectedCustomer ? (
                <div className="space-y-4">
                ...
                <GateMessage message="Select a customer to inspect their orders and operational history." />
              )}
            </Card>
